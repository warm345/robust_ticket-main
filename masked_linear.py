...
class L0Mask(nn.Module):
    def __init__(self, mask_dim, mask_p):
        ...
        self.mask_scores = nn.Parameter(mindspore.zeros(mask_dim))
        self.mask_p = mask_p
    ...
    def produce_mask(self):
        if self.training:
            u = mindspore.zeros_like(self.mask_scores).uniform_().clamp(0.0001, 0.9999)
            s = mindspore.sigmoid((u.log() - (1 - u).log() + self.mask_scores) / self.b)
        else:
            s = mindspore.sigmoid(self.mask_scores)
        s_bar = s * (self.r - self.l) + self.l
        mask = s_bar.clamp(min=0.0, max=1.0)
        return mask
    
    def regularizer(self):
        return mindspore.sum(mindspore.sigmoid(self.mask_scores - self.b * np.log(-self.l / self.r))) / self.mask_scores.numel()
    ...
    def forward(self, input: mindspore.tensor):
        ...
        if self.do_caching:
            if self.cached_activation is None:
                ...
                self.cached_activation = mindspore.cat((
