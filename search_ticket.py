...
from mindspore.optim.lr_scheduler import LambdaLR
...
def set_seed(seed: int):
    ...
    random.seed(seed)
    np.random.seed(seed)
    mindspore.random.manual_seed(seed)
...
def load_data(tokenizer, args):
    ...
    if args.dataset_name == 'imdb' or args.dataset_name == 'ag_news':
        ...
        train_dataset, dev_dataset = mindspore.utils.data.random_split(train_dataset, [train_size, dev_size])
    ...
    for epoch in range(args.epochs):
        ...
        for model_inputs, labels in pbar:
            ...
            if args.adv_init_mag > 0:
                ...
                input_lengths = mindspore.sum(input_mask, 1)
                if args.adv_norm_type == 'l2':
                    delta = mindspore.zeros_like(embedding_init).uniform_(-1, 1) * input_mask.unsqueeze(2)
                    dims = input_lengths * embedding_init.size(-1)
                    magnitude = args.adv_init_mag / mindspore.sqrt(dims)
                    delta = (delta * magnitude.view(-1, 1, 1))
                elif args.adv_norm_type == 'linf':
                    delta = mindspore.zeros_like(embedding_init).uniform_(-args.adv_init_mag,
                                                                      args.adv_init_mag) * input_mask.unsqueeze(2)
            else:
                delta = mindspore.zeros_like(embedding_init)
            ...
            for astep in range(args.adv_steps):
                ...
                # (3) update and clip
                if args.adv_norm_type == "l2":
                    denorm = mindspore.norm(delta_grad.view(delta_grad.size(0), -1), dim=1).view(-1, 1, 1)
                    denorm = mindspore.clamp(denorm, min=1e-8)
                    delta = (delta + args.adv_lr * delta_grad / denorm).detach()
                    if args.adv_max_norm > 0:
                        delta_norm = mindspore.norm(delta.view(delta.size(0), -1).float(), p=2, dim=1).detach()
                    ...
                    denorm = mindspore.norm(delta_grad.view(delta_grad.size(0), -1), dim=1, p=float("inf")).view(-1, 1,
                                                                                                             1)
                    denorm = mindspore.clamp(denorm, min=1e-8)
                    delta = (delta + args.adv_lr * delta_grad / denorm).detach()
                embedding_init = word_embedding_layer(input_ids)
            mindspore.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
        ...
        mindspore.save(args, os.path.join(s, 'training_args.bin'))
        ...
        if accuracy > best_accuracy:
            ...
            mindspore.save(args, os.path.join(output_dir, 'training_args.bin'))
